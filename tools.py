...
def search_hotels(city: str, max_price_per_night: int = 99_999_999) -> str:
    ...
    if not filtered:
        ...
        return f"Không tìm thấy khách sạn tại {city} với giá dưới {price_fmt}đ/đêm. Hãy thử tăng ngân sách."
    ...
    for i, h in enumerate(filtered, 1):
        ...
        stars = "⭐" * h["stars"]
        lines.append(
            f"{i}. {h['name']} {stars} | {price_fmt}đ/đêm | "
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        ...
        for item in expenses.split(","):
            ...
            parts = item.split(":")
            if len(parts) != 2:
                return f"Lỗi định dạng: '{item}' không đúng dạng 'tên_khoản:số_tiền'."
        ...
        for name, amount in expense_dict.items():
            ...
            lines.append(f"  - {display_name}: {amount_fmt}đ")

        lines.append("  ---")
        lines.append(f"  Tổng chi: {total_fmt}đ")
        ...
        if remaining >= 0:
            ...
            lines.append(f"  ⚠️ Vượt ngân sách {over_fmt}đ! Cần điều chỉnh.")

        return "\n".join(lines)

    except ValueError as e:
        return f"Lỗi khi xử lý chi phí: {e}. Vui lòng kiểm tra định dạng số tiền."
